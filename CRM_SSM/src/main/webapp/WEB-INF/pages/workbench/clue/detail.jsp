				...
				$.ajax({
					...
					success:function(data){
						if(data.code == "1"){
							...
							$.each(data.oretData,function (i,n){
								html += '<tr id="tr_'+n.id+'">';
								html += '<td>'+n.name+'</td>';
								html += '<td>'+n.startDate+'</td>';
								html += '<td>'+n.endDate+'<td>';
								html += '<td>'+n.owner+'</td>';
								html += '<td><a href="javascript:void(0);" activityId="'+n.id+'" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
								html += '</tr>';
							})
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		<div class="page-header">
			<h3>${clue.owner}${clue.appellation} <small>${clue.company}</small></h3>
		</div>
		...
		<c:forEach items="${clueRemarkList}" var="clueRemark">
			<div class="remarkDiv" id="div_${clueRemark.id}" style="height: 60px;">
				...
				<div style="position: relative; top: -40px; left: 40px;" >
					...
					<font color="gray">线索</font> <font color="gray">-</font> <b>${clue.owner}${clue.appellation}-${clue.company}</b> <small style="color: gray;"> ${clueRemark.editFlag=="1"?clueRemark.editTime:clueRemark.createTime} 由${clueRemark.editFlag=="1"?clueRemark.editBy:clueRemark.createBy}${remark.editFlag=='1'?'修改':'创建'}</small>
